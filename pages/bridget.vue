...
function getLabels(job: { runner?: string; prompt?: string }): Label[] {
  ...
  if (runner === 'claude') {
    labels.push({ text: 'claude', style: 'bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 border-purple-300 dark:border-purple-800/50' })
  }
  if (runner === 'python' || cmd.includes('python')) {
    labels.push({ text: 'python', style: 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-800/50' })
  }
  if (cmd.includes('ollama') || cmd.includes('eod_nudge') || cmd.includes('ollama_compress')) {
    labels.push({ text: 'ollama', style: 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 border-green-300 dark:border-green-800/50' })
  }
  return labels
}
...
<template>
  <div>
    ...
    <!-- Sync status -->
    <div class="border border-gray-200 dark:border-gray-800 rounded-lg p-4 mb-8">
      <div class="flex items-center justify-between mb-3">
        <span class="text-sm text-gray-500 dark:text-gray-400">Last sync</span>
      ...
      <div v-else class="text-sm text-gray-400 dark:text-gray-600 pt-2 border-t border-gray-200 dark:border-gray-800">Waiting for first sync\u2026</div>
    </div>

    <!-- Scheduled jobs -->
    <h2 class="text-sm text-gray-500 dark:text-gray-400 mb-3">Scheduled jobs</h2>

    <div v-if="jobs === undefined" class="space-y-2">
      <div v-for="i in 4" :key="i" class="h-16 rounded-lg bg-gray-200 dark:bg-gray-900/50 animate-pulse" />
    </div>

    <p v-else-if="jobs.length === 0" class="text-sm text-gray-400 dark:text-gray-600">No scheduled jobs</p>

    <div v-else class="space-y-2">
      <div
        v-for="job in jobs"
        :key="job.name"
        class="border border-gray-200 dark:border-gray-800 rounded-lg p-3 group"
      >
        ...
        <div class="flex items-start justify-between gap-3 mb-2">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <span class="text-xs text-gray-400 dark:text-gray-700">{{ formatDate(job.updatedAt) }}</span>
            <button
              class="text-xs px-2 py-0.5 rounded border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:border-gray-400 dark:hover:border-gray-500 transition-colors font-mono"
        ...
        <div>
          <div v-if="isEditing(job.name, 'schedule')" class="flex items-center gap-2">
            <input
              v-model="editValue"
              class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-gray-900 dark:text-white font-mono text-xs w-full sm:w-44 focus:outline-none focus:border-gray-400 dark:focus:border-gray-400"
              @keyup.enter="saveEdit"
              @keyup.escape="cancelEdit"
            />
            <button :disabled="saving" class="text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 text-xs disabled:opacity-50" @click="saveEdit">save</button>
            <button class="text-gray-500 dark:text-gray-600 hover:text-gray-700 dark:hover:text-gray-400 text-xs" @click="cancelEdit">cancel</button>
          </div>
          <div v-else class="flex items-center gap-2">
            <span class="text-xs font-mono" :class="job.pendingSchedule ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-400 dark:text-gray-600'">
              {{ job.pendingSchedule ? formatCron(job.pendingSchedule) + ' (pending)' : formatCron(job.schedule) }}
            </span>
            <button
              class="opacity-40 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-1 focus-visible:ring-gray-500 outline-none text-gray-500 dark:text-gray-600 hover:text-gray-700 dark:hover:text-gray-400 text-xs transition-opacity rounded px-0.5"
        ...
        <div v-if="job.runner === 'claude'" class="mt-2">
          <div v-if="isEditing(job.name, 'prompt')" class="space-y-2">
            <textarea
              v-model="editValue"
              rows="4"
              class="w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-gray-900 dark:text-white font-mono text-xs focus:outline-none focus:border-gray-400 resize-none"
              @keyup.escape="cancelEdit"
            />
          ...
          <div v-else class="flex items-start gap-2">
            <p class="text-xs font-mono leading-relaxed flex-1 line-clamp-2" :class="job.pendingPrompt ? 'text-yellow-600/70 dark:text-yellow-400/70' : 'text-gray-400 dark:text-gray-600'">
              {{ job.pendingPrompt ? job.pendingPrompt + ' (pending)' : job.prompt }}
            </p>
            <button
              class="opacity-40 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-1 focus-visible:ring-gray-500 outline-none text-gray-500 dark:text-gray-600 hover:text-gray-700 dark:hover:text-gray-400 text-xs transition-opacity shrink-0 rounded px-0.5"
